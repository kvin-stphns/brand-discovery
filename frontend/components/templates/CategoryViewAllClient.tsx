...
import { fetchProducts, type ProductDTO, type ProductFilters } from '@/lib/api/client'
...
interface CategoryViewAllClientProps {
  category: string
  section: string
}
...
export default function CategoryViewAllClient({ category, section }: CategoryViewAllClientProps) {
  const [liveItems, setLiveItems] = useState<ViewAllItem[]>([])
  ...
  const filters = useMemo(() => {
    const f: ProductFilters = { limit: 24 }
  ...
  const loadMore = useCallback(async (reset = false) => {
    ...
    try {
      ...
      const newItems = (prods || []).map((p: ProductDTO, i: number) => ({
        id: String(p._id),
        name: String(p.title || ''),
      ...
      setHasMore(prods.length >= (filters.limit || 24))
    } catch (err) {
