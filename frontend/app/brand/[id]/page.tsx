...
import { fetchBrand, fetchProducts } from '@/lib/api/client'
...
export default function BrandPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchBrand(id),
      fetchProducts({ brandId: /^[0-9a-fA-F]{24}$/.test(id) ? id : undefined, q: /^[0-9a-fA-F]{24}$/.test(id) ? undefined : id, limit: 12 }),
    ])
      .then(([brandDoc, products]) => {
        if (cancelled) return
        setBrand(brandDoc)
  ...
  const name = sanitizeText(brand?.name || (loading ? 'Loading brand' : 'Brand unavailable'))
            ...
            <div className="max-w-2xl pt-6 px-8">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex space-x-4">
                  <button aria-label="Like brand" className="hover:text-[#4FFFF4] transition-colors">
                    <Heart className="w-5 h-5" />
                  </button>
                  <button aria-label="Save brand" className="hover:text-[#4FFFF4] transition-colors">
              ...
              <div className="space-y-6 pb-20">
                <div>
                  ...
                  <p className="text-sm leading-relaxed text-gray-600">
                    {brand ? 'Brand profile data is connected to the live catalog and ready for richer partner-provided copy.' : 'This brand record is not available yet.'}
                  </p>
                </div>
